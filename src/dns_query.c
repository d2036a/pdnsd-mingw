			...
			for(pstart=prt;;) {
				...
				if (bind(s,(struct sockaddr *)&sin,sinl)==-1) {
					...
					if (WSAGetLastError()!=WSAEADDRINUSE &&
						...
						log_warn("Could not bind to socket: %d\n", WSAGetLastError());
#else
					if (errno!=EADDRINUSE &&
...
#ifdef WIN32
			DEBUG_MSG("Could not open socket: %d\n", WSAGetLastError());
#else
...
#ifdef WIN32
			u_long mode = 1;
			if (ioctlsocket(st->sock,FIONBIO,&mode)!=0) {
				DEBUG_PDNSDA_MSG("fcntl error while trying to make socket to %s non-blocking: %d\n", PDNSDA2STR(PDNSD_A(st)),WSAGetLastError());
				closesocket(st->sock);
#else
...
# ifdef WIN32
				if(!run_ipv4 && (WSAGetLastError()==WSAENETUNREACH || WSAGetLastError()==WSAENETDOWN)
# else
...
# ifdef WIN32
					DEBUG_PDNSDA_MSG("Connecting to %s failed: %d, retrying with IPv4 address %s\n",
                                                         PDNSDA2STR(PDNSD_A(st)),WSAGetLastError(),
# else
...
# endif
...
#endif
#ifdef WIN32
				DEBUG_PDNSDA_MSG("Error while connecting to %s: %d\n", PDNSDA2STR(PDNSD_A(st)),WSAGetLastError());
				closesocket(st->sock);
#else
	...
	case QS_TCPWRITE:
		{
			...
			if(rem>0) {
#ifdef WIN32
				rv=send(st->sock,((unsigned char*)st->msg)+st->iolen,rem,0);
#else
				...
				if(rv==-1) {
					...
					if (st->iolen==0 &&
#ifdef WIN32
					    (st->s_errno==WSAECONNREFUSED || st->s_errno==WSAECONNRESET))
#else
...
#ifdef WIN32
						DEBUG_PDNSDA_MSG("TCP connection to %s failed: %d\n", PDNSDA2STR(PDNSD_A(st)),st->s_errno);
#else
...
#ifdef WIN32
					DEBUG_PDNSDA_MSG("Error while sending data to %s: %d\n", PDNSDA2STR(PDNSD_A(st)),st->s_errno);
#else
	...
	case QS_TCPREAD:
			...
			uint16_t recvl_net;
#ifdef WIN32
			rv=recv(st->sock,(char*)&recvl_net,sizeof(recvl_net),0);
			if(rv==-1 && WSAGetLastError()==WSAEWOULDBLOCK)
#else
		...
		}
		{
			...
			if(rem>0) {
#ifdef WIN32
				rv=recv(st->sock,((unsigned char*)st->recvbuf)+offset,rem,0);
#else
	...
	error_receiv_data:
#ifdef WIN32
		if(rv==-1) st->s_errno=WSAGetLastError();
#else
...
# endif
	...
	retry_udp_connect:
		...
		if (connect(st->sock,SOCK_ADDR(st),SIN_LEN)==-1) {
#ifdef WIN32
			if (WSAGetLastError()==WSAECONNREFUSED) st->s_errno=WSAGetLastError();
#else
			...
			/* if IPv6 connectivity is for some reason unavailable, perhaps the
			   IPv4 fallback address can still be reached. */
# ifdef WIN32
			else if(!run_ipv4 && (WSAGetLastError()==WSAENETUNREACH || WSAGetLastError()==WSAENETDOWN)
# else
...
# endif
		 ...
		 * must be empty) */
		if (send(st->sock,(char*)&st->msg->hdr,st->transl,0)==-1) {
#ifdef WIN32
			st->s_errno=WSAGetLastError();
			DEBUG_PDNSDA_MSG("Error while sending data to %s: %d\n", PDNSDA2STR(PDNSD_A(st)),WSAGetLastError());
			closesocket(st->sock);
	...
	case QS_UDPRECEIVE:
	{
		...
		if ((rv=recv(st->sock,(char*)st->recvbuf,udpbufsize,0))==-1) {
#ifdef WIN32
			st->s_errno=WSAGetLastError();
			DEBUG_PDNSDA_MSG("Error while receiving data from %s: %d\n", PDNSDA2STR(PDNSD_A(st)),WSAGetLastError());
			closesocket(st->sock);
#else
			st->s_errno=errno;
			DEBUG_PDNSDA_MSG("Error while receiving data from %s: %s\n", PDNSDA2STR(PDNSD_A(st)),strerror(errno));
		...
		if (rv!=RC_OK) {
			...
			if(st->needs_testing) {
				...
				case WSAENETUNREACH:  /* network unreachable */
                                ...
                                case WSAENOPROTOOPT:  /* protocol unreachable */
                                case WSAECONNREFUSED: /* port unreachable */
                                case WSAENETDOWN:     /* network down */
				case WSAEHOSTDOWN:    /* host down */
...
# endif
					...
					if (nevents<0) {
						/* if(errno==EINTR)
							continue; */
#ifdef WIN32
						log_warn("select failed: %d",WSAGetLastError());
#else
...
# endif
			...
			*/
			...
			if (event<0) {
#ifdef WIN32
				if(WSAGetLastError()==WSAEINTR && is_interrupted_servstat_thread()) {
#else

...
#ifdef WIN32
		msg.name=(LPSOCKADDR)&((udp_buf_t *)data)->addr.sin4;
...
#   ifdef WIN32
		msg.Control.len=CMSG_SPACE(sizeof(struct in_addr));
#   else
...
# ifdef WIN32
		msg.name=(LPSOCKADDR)&((udp_buf_t *)data)->addr.sin6;
...
#  ifdef WIN32
		msg.Control.len=CMSG_SPACE(sizeof(struct in6_pktinfo));
#  else
...
#endif
#ifdef WIN32
	if (WSASendTo(((udp_buf_t *)data)->sock,
			msg.lpBuffers,msg.dwBufferCount,
			&bytes_sent,msg.dwFlags,
			msg.name,msg.namelen,
			NULL,NULL)<0) {
#else
...
#ifdef WIN32
			log_error("Error in udp send: %d",WSAGetLastError());
#else
...
# ifdef WIN32
			log_error("Could not open udp socket: %d",WSAGetLastError());
# else
...
# endif
# ifdef WIN32
			log_error("Could not set options on udp socket: %d",WSAGetLastError());
                        closesocket(sock);
# else
...
#ifdef WIN32
                log_error("Could not bind to udp socket: %d",WSAGetLastError());
                closesocket(sock);
#else
...
void *udp_server_thread(void *dummy)
{
	...
	while (1) {
		...
		nResult = WSAIoctl(sock, SIO_GET_EXTENSION_FUNCTION_POINTER,
                ...
                if(nResult!=0)
                {
                	log_error("get WSARecvMsg() function point failed.%d\n",WSAGetLastError());
                	goto free_buf_continue;
                }

		data_buf.buf=(char *)buf->buf;
		data_buf.len=udpbufsize;
		msg.lpBuffers=&data_buf;
		msg.dwBufferCount=1;
		msg.Control.buf=ctrl;
		msg.Control.len=sizeof(ctrl);
...
# ifdef ENABLE_IPV4
		if (run_ipv4) {
#ifdef WIN32
			msg.name=(LPSOCKADDR)&buf->addr.sin4;
                        msg.namelen=sizeof(struct sockaddr_in);
			if (WSARecvMsg(sock,&msg,(LPDWORD)&qlen,NULL,NULL)==0){
#else
...
#  endif
				if (++da_udp_errs<=UDP_MAX_ERRS) {
#  ifdef WIN32
					log_error("error in UDP recv: %d", WSAGetLastError());
#  else
...
# ifdef ENABLE_IPV6
		ELSE_IPV6 {
#  ifdef WIN32
			msg.name=(LPSOCKADDR)&buf->addr.sin6;
			msg.namelen=sizeof(struct sockaddr_in6);
			if (WSARecvMsg(sock,&msg,(LPDWORD)&qlen,NULL,NULL)==0){
#  else
...
# endif
...
#endif
		{
			...
			uint16_t rlen_net;
#ifdef WIN32
			if ((err=recv(sock,(char*)&rlen_net,sizeof(rlen_net),0))!=sizeof(rlen_net)) {
#else
...
#endif
#ifdef WIN32
			rv=recv(sock,buf+olen,rlen-olen,0);
#else
...
#ifdef WIN32
				DEBUG_MSG("Error while writing to TCP client: %d\n",WSAGetLastError());
#else
...
# ifdef WIN32
			log_error("Could not open tcp socket: %d",WSAGetLastError());
# else
...
#ifdef WIN32
			log_warn("Could not set options on tcp socket: %d",WSAGetLastError());
#else
...
#ifdef WIN32
                log_error("Could not bind tcp socket: %d",WSAGetLastError());
                closesocket(sock);
#else
...
#ifdef WIN32
			log_error("Could not listen on tcp socket: %d",WSAGetLastError());
#else
...
#ifdef WIN32
			if (WSAGetLastError()!=WSAEINTR && ++da_tcp_errs<=TCP_MAX_ERRS) {
				log_error("tcp accept failed: %d",WSAGetLastError());
#else

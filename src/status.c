			...
			if (read_short(rs,&cmd)) {
				...
				case CTL_STATS: {
					...
					DWORD dwSize=sizeof(buffer);
                                        COMPUTER_NAME_FORMAT cnf=ComputerNameDnsHostname;
					GetComputerNameEx(cnf,buffer,&dwSize);
					nodename=buffer;
#else
					struct utsname nm;
...
#ifdef WIN32
/*
 * Initialize the status named pipe
 */
void init_stat_sock()
{
	LPTSTR lpszPipename = TEXT("\\\\.\\pipe\\pdnsd.status");
	
	if ((stat_sock=CreateNamedPipe( 
                ...
                log_warn("Failed to create named pipe: %d. Status readback will be impossible",GetLastError());
                stat_pipe=0;
                return;
        }
}

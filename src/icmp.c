...
{
#ifdef WIN32
	if ((HANDLE)(ping_isocket=(int)IcmpCreateFile())==INVALID_HANDLE_VALUE) {
		log_warn("icmp ping: IcmpCreateFile() failed: %d",GetLastError());
#else
...
# ifdef WIN32
		if ((HANDLE)(ping6_isocket=(int)Icmp6CreateFile())==INVALID_HANDLE_VALUE) {
			log_warn("icmpv6 ping: Icmp6CreateFile() failed: %d",GetLastError());
# else
...
#ifdef WIN32
/*  For sendind ICMP packet need to create a socket of type SOCK_RAW, but nly members of the 
 *  Administrators group can create sockets of type SOCK_RAW on Windows 2000 and later.
 *  For this using IcmpSendEcho instead which not need Administrator Privileges. */
static int ping4(struct in_addr addr, int timeout, int rep)
{
        ...
        hIcmpFile=(HANDLE)ping_isocket;

        for (i=0;i<rep;i++) {
                long tm=time(NULL);
                if((IcmpSendEcho(hIcmpFile,addr.S_un.S_addr,NULL,0,NULL,&buf,sizeof(buf),timeout))==0)
                {
                        if (++icmp_errs<=ICMP_MAX_ERRS) {
                                log_warn("icmp ping: IcmpSendEcho() failed: %d", GetLastError());
                        }
                        return -1;
                }
                switch (buf.Status) {
                        case IP_SUCCESS:
                                return (i*timeout+time(NULL)-tm); /* Windows can not get icmp_seq any way */
                        case IP_REQ_TIMED_OUT:
                                break; /* timed out */
                        default:
                                DEBUG_MSG("icmp reply error:%d",buf.Status);
                                return -1;
                }
        }
        return -1; /* no answer */
...
/*  For sendind ICMP packet need to create a socket of type SOCK_RAW, but nly members of the 
 *  Administrators group can create sockets of type SOCK_RAW on Windows 2000 and later.
 *  For this using Icmp6SendEcho2 instead which not need Administrator Privileges. */
static int ping6(struct in6_addr a, int timeout, int rep)
{
	int i;
        HANDLE hIcmpFile;
	struct sockaddr_in6 to;
        ICMPV6_ECHO_REPLY buf;

        hIcmpFile=(HANDLE)ping6_isocket;
	memset(&to,0,sizeof(to));
        to.sin6_family=AF_INET6;
        to.sin6_flowinfo=IPV6_FLOWINFO;
        to.sin6_port=0;
        to.sin6_addr=a;
        SET_SOCKA_LEN6(to);
		...
		long tm=time(NULL);
				...
				NULL,		/* RequestOptions */
				&buf,		/* ReplyBuffer */
				sizeof(buf),	/* ReplySize */
				timeout))==0)
                {
                        if (++icmp_errs<=ICMP_MAX_ERRS) {
                                log_warn("icmp ping: Icmp6SendEcho2() failed: %d", GetLastError());
                        }
                        return -1;
                }
                switch (buf.Status) {

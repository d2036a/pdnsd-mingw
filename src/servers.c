...
static int uptest (servparm_t *serv, int j)
{
	...
	pdnsd_a *addr_s= PDNSD_A2_TO_A(&DA_INDEX(serv->atup_a,j).a);

	DEBUG_PDNSDA_MSG("performing uptest (type=%s) for %s\n",const_name(serv->uptest),PDNSDA2STR(addr_s));
	...
	case C_NONE:
		...
		break;
	case C_PING:
		ret=ping(is_inaddr_any(&serv->ping_a) ? addr_s : &serv->ping_a, serv->ping_timeout,PINGREPEAT)!=-1;
		break;
	case C_IF:
 	case C_DEV:
	case C_DIALD:
 		ret=if_up(serv->ifname);
#if (TARGET==TARGET_LINUX)
 		if (ret!=0) {
			if(serv->uptest==C_DEV)
				ret=dev_up(serv->ifname,serv->device);
	...
	case C_EXEC: {
#ifdef WIN32
		if(spawnlp(P_WAIT,"cmd.exe","/c",serv->uptest_cmd,NULL)==0)
			ret=1;
#else
	  	pid_t pid;
		...
		break;
	case C_QUERY:
		ret=query_uptest(addr_s, serv->port, serv->query_test_name,
...
#ifdef WIN32
		if(!PathMatchSpec(schm,serv->scheme))
#else
...
#ifdef WIN32
		if((err=pthread_kill(servstat_thrid,0))) {
#else
